import userFeedReducer, {getUserFeed, TUserFeedState} from './user-feed-slice';
...
describe('Проверяем редьюсер слайса userFeedSlice', () => {
  ...
  test('Обработка getUserFeed.pending', () => {
    const newState = userFeedReducer({...initialState, error: 'error'}, getUserFeed.pending(''));
    expect(newState).toEqual(statePending);
  });

   test('Обработка getUserFeed.fulfilled', () => {
    const newState = userFeedReducer({...initialState, loading: true}, getUserFeed.fulfilled([testOrders], ''));
  ...
  test('Обработка getUserFeed.rejected', () => {
    ...
    const newState = userFeedReducer({...initialState, loading: true}, getUserFeed.rejected(testError, ''));
