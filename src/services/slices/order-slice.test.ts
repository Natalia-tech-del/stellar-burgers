import orderReducer, {postOrder, getOrderByNumber, TOrderState} from './order-slice';
...
describe('Проверяем редьюсер слайса orderSlice', () => {
  ...
  test('Обработка postOrder.pending', () => {
    const newState = orderReducer({...initialState, error: 'error'}, postOrder.pending('', []));
    expect(newState).toEqual(statePending);
  });

  test('Обработка getOrderByNumber.pending', () => {
    const newState = orderReducer({...initialState, error: 'error'}, getOrderByNumber.pending('', 0));
    expect(newState).toEqual(statePending);
  });

  test('Обработка postOrder.fulfilled', () => {
    const newState = orderReducer({...initialState, orderRequest: true}, postOrder.fulfilled(testOrders, '', testOrders.ingredients));
    expect(newState).toEqual(statePostOrderFulfilled);
  });

  test('Обработка getOrderByNumber.fulfilled', () => {
    const newState = orderReducer({...initialState, orderRequest: true}, getOrderByNumber.fulfilled(testOrders, '', 1));
    expect(newState).toEqual(stateGetOrderByNumberFulfilled);
  });

  test('Обработка postOrder.rejected', () => {
    const testError = new Error('Error')
    const newState = orderReducer({...initialState, orderRequest: true}, postOrder.rejected(testError, '', []));
    expect(newState).toEqual(stateRejected);
  });

  test('Обработка getOrderByNumber.rejected', () => {
    const testError = new Error('Error')
    const newState = orderReducer({...initialState, orderRequest: true}, getOrderByNumber.rejected(testError, '', 0));
    expect(newState).toEqual(stateRejected);
  });
});
